...
class Solution {
  public:
    vector<int> longestSubarray(vector<int>& arr, int x) {
        ...
        for(int i =0;i<n;i++){
            while(!minQ.empty() and arr[minQ.back()]>arr[i]) minQ.pop_back();
            while(!maxQ.empty() and arr[maxQ.back()]<arr[i]) maxQ.pop_back();
            ...
            if((i-start)>resEnd-resStart) resStart = start,resEnd=i;
